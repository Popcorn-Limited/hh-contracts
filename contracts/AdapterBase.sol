...
import { IAdapter, IERC4626 } from "./interfaces/IAdapter.sol";
    ...
    OnlyStrategy
{
    ...
    function _verifyAndSetupStrategy(bytes4[8] memory requiredSigs) internal {
        ...
        (bool success, ) = address(strategy).delegatecall(abi.encodeWithSignature("setUp(bytes)", strategyConfig));
    ...
    function accruedPerformanceFee() public view returns (uint256) {
        ...
        return
            performanceFee_ > 0 && shareValue > highWaterMark_
                ? performanceFee_.mulDiv((shareValue - highWaterMark_) * totalSupply(), 1e36, Math.Rounding.Floor)
                : 0;
    }
