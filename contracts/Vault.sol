...
import { VaultFees, IERC4626, IERC20 } from "./interfaces/IVault.sol";

/**vscode-remote://wsl%2Bubuntu/home/veil/popcorn/contracts/src/interfaces/vault/IVault.sol
 * @title   Vault
 * @author  RedVeil
...
contract Vault is ERC4626Upgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, OwnedUpgradeable {
    ...
    function mint(
        ...
        uint256 feeShares = shares.mulDiv(depositFee, 1e18 - depositFee, Math.Rounding.Floor);
    ...
    function withdraw(
        ...
        uint256 feeShares = shares.mulDiv(withdrawalFee, 1e18 - withdrawalFee, Math.Rounding.Floor);
    ...
    function redeem(
        ...
        uint256 feeShares = shares.mulDiv(uint256(fees.withdrawal), 1e18, Math.Rounding.Floor);
    ...
    function previewDeposit(uint256 assets) public view override returns (uint256 shares) {
        shares = adapter.previewDeposit(assets - assets.mulDiv(uint256(fees.deposit), 1e18, Math.Rounding.Floor));
    }
    ...
    function previewMint(uint256 shares) public view override returns (uint256 assets) {
        ...
        shares += shares.mulDiv(depositFee, 1e18 - depositFee, Math.Rounding.Floor);
    ...
    function previewWithdraw(uint256 assets) public view override returns (uint256 shares) {
        ...
        assets += assets.mulDiv(withdrawalFee, 1e18 - withdrawalFee, Math.Rounding.Floor);
    ...
    function previewRedeem(uint256 shares) public view override returns (uint256 assets) {
        ...
        assets -= assets.mulDiv(uint256(fees.withdrawal), 1e18, Math.Rounding.Floor);
    }
    ...
    function accruedPerformanceFee() public view returns (uint256) {
        ...
        return
            performanceFee > 0 && shareValue > highWaterMark_
                ? performanceFee.mulDiv((shareValue - highWaterMark_) * totalSupply(), 1e36, Math.Rounding.Floor)
                : 0;
    }
    ...
    modifier takeFees() {
        ...
        if (totalFee > 0 && currentAssets > 0) {
            ...
            uint256 feeInShare = supply == 0
                ? totalFee
                : totalFee.mulDiv(supply, currentAssets - totalFee, Math.Rounding.Floor);
